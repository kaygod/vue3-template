...
export const Confirm = (props:{message:string}) => {
  return new Promise((resolve, reject) => {
    ElMessageBox.confirm(
      ...
      .catch(() => {
        // Info("删除取消了");
        reject(new Error('delete is cancel'))
      })
  })
}
    ...
    axios({
      ...
      .then((res: AxiosResponse<T>) => {
        ...
        if (rResult && rResult.result) {
          +rResult.result === 1 && resolve(rResult.data)
        } else {
          closeLoading()
          if (rResult.errno != null) {
            const errno = rResult.errno.toString()
            if (errno === '1030' || errno === '1040') {
              // 没有登录 或者 被挤下来了
...
export const paramsHandler = function (v: any) {
  ...
  let second = params2.slice(1)
  second += params2[0]
  params2 = second
  const reg = new RegExp(secrete)
  params = params.replace(reg, params2)
  return params
}

function getLoginCode () {
  const loginStateData = localStorage.getItem('loginState')

  if (loginStateData) {
    const login_state = JSON.parse(loginStateData)
